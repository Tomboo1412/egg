...
class ocr:
    ...
    def top_view_shot(self,image_msg):
        ...
        # 从参数服务器获取ocr_det的�?        ocr_det = rospy.get_param('/ocr_det', 255)
        
        if ocr_det == 1:
            # 保存图像
            rospy.loginfo('保存至temp/ocr_now.jpg')
            cv2.imwrite('/home/abot/egg/egg_ws/src/ocr_detect/temp/ocr_now.jpg', self.img_bgr)
            # 将ocr_det重置�?55
            ...
            # 调用ocr模型识别
            #ocr_detection()
    def ocr_detection( self,img_path='/home/abot/egg/egg_ws/src/ocr_detect/temp/ocr_now.jpg'):
            global ocr1
            while True:
                if self.is_image_complete('/home/abot/egg/egg_ws/src/ocr_detect/temp/ocr_now.jpg'):
                    break
                else:
                    cv2.imwrite('/home/abot/egg/egg_ws/src/ocr_detect/temp/ocr_now.jpg', self.img_bgr)
            result = ocr1.ocr(img_path)
            combined_text = ""  # 创建一个空字符串来存储所有句�?            count = 0  # 初始化计数器
            for temp in result:
                if count >= 2:  # 如果计数器达�?2，跳出循�?                    break
                combined_text += temp["text"] + " "  # 将每个句子添加到 combined_text 中，并在每个句子后添加一个空�?                count += 1  # 计数器加 1
            rospy.loginfo(combined_text.strip())  # 输出合并后的句子，并去掉末尾多余的空�?            os.remove(img_path)
...
def main():
    ...
    rospy.loginfo('准备识别...')
    # 从参数服务器获取ocr_det的�?    ocr_det = rospy.set_param('/ocr_det', 255)
    # 创建服务服务�?    s = rospy.Service('ocr_detection', Trigger, handle_ocr_detection)
