...
def create_blank_img(img_w, img_h):
    # 将图像宽度调整为原来�?.5�?    img_w = int(img_w * 1.5)
...
def Draw_OCRResult(blank_img, pt1, pt2, text):
    # 确保 blank_img �?PIL.Image 对象
    if isinstance(blank_img, np.ndarray):
        blank_img = Image.fromarray(blank_img)
    #创建绘制图像
    draw = ImageDraw.Draw(blank_img)
    #绘制矩形�?    draw.rectangle([pt1, pt2], outline=(255, 255, 0), width=3)
    #加载中文字体（指定字体路径、大小和编码�?    fontStyle = ImageFont.truetype("/home/abot/egg/egg_ws/src/ocr_detect/ChineseFonts/simsun.ttc", size=21, encoding="utf-8")
...
def ocr_detect():
    ...
    img_path = rospy.get_param('image_path', "/home/abot/egg/egg_ws/src/ocr_detect/image/11.jpg")
    ...
    while not rospy.is_shutdown():
        ...
        # 保存图片
        # 提取文件名部�?        base_name = os.path.basename(img_path)
        file_name, _ = os.path.splitext(base_name)
        save_path = f'/home/abot/egg/egg_ws/src/ocr_detect/result/OCR_result_{file_name}.jpg'
